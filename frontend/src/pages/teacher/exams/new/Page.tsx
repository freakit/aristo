...
import { useExamCreateLogic } from "./hooks/useExamCreateLogic";
  ...
  return (
    <S.PageContainer>
      <form onSubmit={handleSubmit}>
        <S.PageHeader>
          <S.HeaderContent>
            <S.HeaderIcon>{isEdit ? "✏️" : "📝"}</S.HeaderIcon>
            <S.HeaderText>
              <S.PageTitle>
                {isEdit ? "시험 수정" : "새 시험 만들기"}
              </S.PageTitle>
              <S.PageSubtitle>
                {isEdit
                  ? "기존 시험의 내용을 수정하고 관리합니다."
                  : "학생들을 위한 새로운 시험을 생성하고 배포합니다."}
              </S.PageSubtitle>
            </S.HeaderText>
          </S.HeaderContent>
          <S.ActionButtons>
            ...
            >
              {isSubmitting ? "저장 중..." : isEdit ? "수정 완료" : "시험 생성"}
            </S.Button>
          </S.ActionButtons>
        </S.PageHeader>
                  ...
                  >
                    ...
                    >
                      <div>
                        ...
                        <S.StudentInfo>{student.school}</S.StudentInfo>
                        <S.StudentInfo>
              ...
              <S.SectionContainer key={section.id}>
                <S.SectionHeader>
                  <S.SectionTitle>
                    ...
                    {isEdit ? "시험 내용" : `섹션 ${index + 1}`}
                  </S.SectionTitle>
                  {!isEdit && sections.length > 1 && (
                ...
                <S.FormGroup>
                  <S.Label>
                    <S.LabelIcon>📚</S.LabelIcon> 참고 자료 (RAG Sources)
                    <span
                      style={{
                        fontSize: "12px",
                        color: "#64748b",
                  ...
                  <div style={{ marginBottom: "10px" }}>
                    <S.Button
                      ...
                      size="small"
                      onClick={() => {
                        // Using a simple prompt or custom modal logic here would be ideal.
                        // For now, let's use a details/summary or inline expansion to show the list.
                        // Or better, let's just show the list with better UI (Table-like).
                        // Since I can't easily add a complex modal state without re-rendering the whole page logic hook,
                        // I will use an inline scrollable list with better formatting.
                      }}
                      style={{ display: "none" }} // Hidden for now, using inline list
                    >
                      소스 선택
                    </S.Button>
                  </div>
                      ...
                      >
                          ...
                          <tr>
                            ...
                            >
                              <input
                                type="checkbox"
                                onChange={(e) => {
                                  const allIds = availableSources.map(
                                    (s) => s.file_id,
                                  );
                                  updateSection(
                                    index,
                        ...
                        <tbody>
                          {availableSources.map((source) => {
                            const isChecked = (
                              section.ragSourceIds || []
                            ).includes(source.file_id);
                            return (
                              <tr
                                key={source.file_id}
                                style={{
