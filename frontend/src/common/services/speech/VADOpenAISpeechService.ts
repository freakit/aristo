// src/services/speech/VADOpenAISpeechService.ts
// VAD 자동 감지 + OpenAI Whisper STT (참조 구현)

import { sendAnswerToAI } from "@/common/services/student/sendAnswerToAI";
import { speechToText } from "@/common/services/student/speechToText";
...
export class VADOpenAISpeechService implements ISpeechService {
  public readonly sttMode: STTMode = "vad_openai";
  ...
  constructor() {
    if ("speechSynthesis" in window) {
      ...
      console.log(
        `🎤 VAD OpenAI Speech Service Initialized (Mode: vad_openai)`,
      );
    } else {
  ...
  public disconnect(): void {
    ...
    console.log("🔌 VAD OpenAI Speech Service Disconnected");
  }
  ...
  private async initializeClientVAD() {
    try {
        ...
        try {
          ...
          if (sttResponse.success) {
            if (sttResponse.data?.transcript) {
              console.log("✅ STT Result:", sttResponse.data.transcript);
      ...
      console.log("✅ VAD Initialized (Waiting for resumeVAD to start)");
    } catch (error) {
  ...
  private handleError(key: string, error?: any) {
    ...
    console.error(`[VAD OpenAI SpeechService Error] ${key}`, errorMessage);
